"""
index.py — веб-інтерфейс для Бібліотечної системи
...
# Глобальний екземпляр бібліотеки (зберігається поки сервер працює)
lib = Library()

# Додаємо демо-дані при старті
...
<div class="stats-bar">
  <div class="stat">
    <div class="stat-num">{{ report.total_books }}</div>
    <div class="stat-label">Книг всього</div>
  </div>
  <div class="stat-divider"></div>
  <div class="stat">
    <div class="stat-num">{{ report.available_books }}</div>
    <div class="stat-label">Доступних</div>
  </div>
  <div class="stat-divider"></div>
  <div class="stat">
    <div class="stat-num">{{ report.total_readers }}</div>
    <div class="stat-label">Читачів</div>
  </div>
  <div class="stat-divider"></div>
  <div class="stat">
    <div class="stat-num">{{ report.active_rentals }}</div>
    <div class="stat-label">Активних оренд</div>
  </div>
  {% if report.overdue_rentals > 0 %}
  <div class="stat-divider"></div>
  <div class="stat">
    <div class="stat-num overdue">{{ report.overdue_rentals }}</div>
    <div class="stat-label overdue">Прострочених</div>
  </div>
  {% endif %}
</div>
...
<div class="main">
  ...
  <div class="panel">
        ...
        {% if book.is_available %}
          ...
          <form method="POST" action="/rent">
            ...
            <select name="reader_id" style="width:auto; font-size:0.72rem; padding:0.2rem 0.4rem; margin-bottom:0.3rem;">
              {% for r in readers %}
              <option value="{{ r.reader_id }}">{{ r.name }}</option>
              {% endfor %}
            </select>
            <button type="submit" class="btn btn-primary" style="padding:0.25rem 0.6rem; font-size:0.65rem;">
              Видати
            </button>
          </form>
        {% else %}
    ...
    <div class="form-section" style="margin-top:1.5rem;">
      ...
      <form method="POST" action="/add_book">
        <div class="form-row">
          <label>Назва</label>
          <input type="text" name="title" required placeholder="Назва книги">
        </div>
        <div class="form-row">
          <label>Автор</label>
          <input type="text" name="author" required placeholder="Прізвище Ім'я">
        </div>
        <div class="form-row" style="display:grid; grid-template-columns:1fr 1fr; gap:0.5rem;">
          ...
          <div>
            <label>Рік</label>
            <input type="number" name="year" placeholder="2024" min="0" max="2100">
          </div>
        </div>
  ...
  <div class="panel">
    ...
    <div class="reader-card">
      <div style="display:flex; justify-content:space-between; align-items:start;">
        <div>
          ...
          {% if reader.rented_book_ids %}
          <div class="reader-books">📚 {{ reader.rented_book_ids | length }} книг(и) на руках</div>
          {% endif %}
        </div>
        <div style="display:flex; gap:0.4rem; align-items:center;">
          {% if reader.is_blocked %}
            ...
            <form method="POST" action="/unblock">
              ...
              <button class="btn btn-success" style="padding:0.2rem 0.5rem; font-size:0.65rem; margin:0;">
                Розблок.
              </button>
            </form>
          {% else %}
            ...
            <form method="POST" action="/block">
              <input type="hidden" name="reader_id" value="{{ reader.reader_id }}">
              <button class="btn btn-danger" style="padding:0.2rem 0.5rem; font-size:0.65rem; margin:0;">
                Блок.
              </button>
            </form>
          {% endif %}
    ...
    <!-- Форма реєстрації читача -->
    <div class="form-section" style="margin-top:1.5rem;">
      <h3>＋ Зареєструвати читача</h3>
      <form method="POST" action="/add_reader">
        <div class="form-row">
          <label>Ім'я</label>
          <input type="text" name="name" required placeholder="Повне ім'я">
        </div>
        <div class="form-row">
          <label>Email</label>
          <input type="email" name="email" required placeholder="reader@example.com">
        </div>
  ...
  <div class="panel">
        ...
        <div class="rental-item {% if record.is_overdue %}overdue{% endif %}">
          <div>
            <div style="font-weight:500; font-size:0.82rem;">
              {{ book_map.get(record.book_id, '?') }}
            </div>
            <div style="opacity:0.55; font-size:0.72rem; margin-top:0.1rem;">
              {{ reader_map.get(record.reader_id, '?') }}
            </div>
            <div style="opacity:0.45; font-size:0.7rem;">
              до {{ record.due_date.strftime('%d.%m.%Y') }}
            </div>
          </div>
          {% if record.is_overdue %}
          <span class="badge badge-rented">прострочено</span>
          {% endif %}
        </div>
      {% endif %}
    {% endfor %}
    ...
    {% if not has_active %}
    <div class="empty-state">Активних оренд немає</div>
    {% endif %}
    ...
    <div class="rental-item" style="opacity:0.5;">
      ...
      <div style="font-size:0.7rem;">
        {{ record.return_date.strftime('%d.%m.%Y') if record.return_date else '' }}
      </div>
    </div>
    {% endfor %}
    {% endif %}
...
def index():
    ...
    # Словники для відображення назв у списку оренд
