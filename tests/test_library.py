# test_library.py
# Модульні тести для бібліотечної системи
...
class TestCatalog:
    ...
    def test_05_add_book_returns_book_object(self):
        """add_book повертає об'єкт Book з правильними полями."""
        book = self.catalog.add_book("1984", "Джордж Орвелл", "Антиутопія", 1949)
        assert isinstance(book, Book)
        assert book.title == "1984"
        assert book.author == "Джордж Орвелл"

    def test_06_add_book_increments_total_count(self):
        """Після додавання книги total_count збільшується."""
    ...
    def test_10_get_book_nonexistent_raises_error(self):
        """get_book з неіснуючим ID викликає KeyError."""
        with pytest.raises(KeyError):
            self.catalog.get_book(999)

    def test_11_search_by_title_partial_match(self):
        """Пошук за назвою знаходить часткове співпадіння."""
    ...
    def test_12_get_available_books_filters_correctly(self):
        """get_available_books повертає лише доступні книги."""
        ...
        assert available[0].title == "Доступна"
...
class TestReaderRegistry:
    ...
    def test_18_register_duplicate_email_raises_error(self):
        """Повторна реєстрація з тим самим email викликає ValueError."""
    ...
    def test_19_register_invalid_email_raises_error(self):
        """Некоректний email викликає ValueError."""
        with pytest.raises(ValueError):
            self.registry.register("Користувач", "not-an-email")

    def test_20_block_and_unblock_reader(self):
        """Блокування та розблокування читача працює коректно."""
...
class TestRentalRecord:
    ...
    def test_23_overdue_rental_detected(self):
        """Прострочена оренда (дата повернення минула) визначається як overdue."""
    ...
    def test_24_returned_rental_not_overdue_if_on_time(self):
        """Своєчасно повернута книга не є простроченою."""
...
class TestLibraryIntegration:
    """Інтеграційні тести класу Library."""

    def setup_method(self):
        self.lib = Library()

    def test_27_rent_and_return_full_cycle(self):
        """Повний цикл: додавання → реєстрація → видача → повернення."""
        book = self.lib.add_book("Дюна", "Герберт", "Фантастика", 1965)
        reader = self.lib.register_reader("Олег Олегов", "oleg@test.com")

        record = self.lib.rent_book(book.book_id, reader.reader_id)
        assert not book.is_available
        assert book.book_id in reader.rented_book_ids

        self.lib.return_book(book.book_id)
        assert book.is_available
        assert book.book_id not in reader.rented_book_ids

    def test_28_rent_unavailable_book_raises_error(self):
        """Видача вже виданої книги викликає ValueError."""
    ...
    def test_29_status_report_reflects_state(self):
        """Звіт коректно відображає поточний стан бібліотеки."""
    ...
    def test_30_mock_rental_manager_called_on_rent(self):
        """Mock-тест: при видачі книги викликається create_rental менеджера оренди."""
        book = self.lib.add_book("Мок-книга", "Автор", "Жанр", 2022)
        reader = self.lib.register_reader("Мок-читач", "mock@test.com")

        mock_record = MagicMock()
        mock_record.rental_id = 999
        self.lib.rentals.create_rental = MagicMock(return_value=mock_record)

        self.lib.rent_book(book.book_id, reader.reader_id)

        self.lib.rentals.create_rental.assert_called_once_with(book.book_id, reader.reader_id)
